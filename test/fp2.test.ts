...
import { Fp, Fp2, verifyBatch } from '..';
...
describe('bls12-381 Fp2', () => {
	...
	it('sqrt_cyon', () => {
		let a = new Fp2(
			new Fp(784063022264861764559335808165825052288770346101304131934508881646553551234697082295473567906267937225174620141295n),
			new Fp(1488924004771393321054797166853618474668089414631333405711627789629391903630694737978065425271543178763948256226639n)
		);
		expect(a.sqrt()?.square()).toEqual(a);

		
		let b = new Fp2(
			new Fp(1367714067195338330005789785234579356639813143898609672616051936515282693167588605474094216909910046701175380147690n),
			new Fp(0n)
		);
		expect(b.sqrt()?.square()).toEqual(b);

		let c = new Fp2(
			new Fp(25n),
			new Fp(0n)
		);
		expect(c.sqrt()?.square()).toEqual(c);
	...
	it('many squares', () => {
		manySquares.forEach((v) => {
			const input = Fp2.fromBigTuple([BigInt(v.input.c0), BigInt(v.input.c1)]);
			const expectedSquared = Fp2.fromBigTuple([BigInt(v.squared.c0), BigInt(v.squared.c1)]);
			const squared = input.square();
			expect(squared.equals(expectedSquared)).toBe(true);
		});

	});

	it('frobenius', () => {
		const a = Fp2.fromBigTuple([
		...
		const b = a.frobeniusMap(0);
		expect(a.equals(b)).toBe(true);
		
		const c = b.frobeniusMap(1);
		expect(
			c.equals(
				Fp2.fromBigTuple([
					b.c0.value,
		...
		const d = c.frobeniusMap(1);
		expect(d.equals(a)).toBe(true);

		const e = d.frobeniusMap(2);
		expect(e.equals(a)).toBe(true);
	});
});
